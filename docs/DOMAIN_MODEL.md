...
Справочники. `Specialization` принадлежит `Field`. `Skill` связан с одной или несколькими `Field` и используется в `UserSkill` и `ProjectRoleSkill`.

`Field`, `Specialization` и `Skill` — системные справочники, управляются через админку/seed/служебные инструменты и не создаются пользователем через публичный API. `Field.is_featured` и `Field.featured_order` используются для featured-блока на главной; `featured_order` — внутреннее поле сортировки. "Все профили" не является записью `Field` в базе, это фронтовая синтетическая карточка.

`Skill.fields` нужен для frontend autocomplete/filter UX: после выбора области frontend может запросить `GET /skills/?field_ids=...` и показать релевантные навыки. Matching по-прежнему строится вокруг специализации пользователя и ролей проекта, а навыки уточняют профиль и требования ролей.

`User.city` — свободная строка профиля, не системный справочник. Backend не валидирует город по списку; frontend может использовать локальные autocomplete-подсказки и должен позволять сохранить пользовательский вариант.
