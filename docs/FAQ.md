...
Как обновлять JWT-токены?

### Ответ

Отправить текущий refresh token в `POST /api/v1/auth/token/refresh/`. Endpoint возвращает новый `access`, новый `refresh` и `user`. После успешного обновления клиент должен сохранить оба новых токена; старый refresh token повторно использовать нельзя.
...
`Field`, `Specialization` и `Skill` нельзя создавать через публичный API; ими управляют через админку, seed или служебные инструменты. `GET /api/v1/skills/` публичен, но `POST /api/v1/skills/` не является пользовательским сценарием.
...
Поиск контекстный: для проектов фронт вызывает `GET /api/v1/projects/?search=...`, для участников — `GET /api/v1/users/?search=...`. Отдельный глобальный `/search/` endpoint не входит в MVP. Multi-value filters `field_ids`, `skill_ids` и `specialization_ids` работают как OR внутри группы; разные группы фильтров применяются совместно.

### Вопрос

Город — это backend-справочник?

### Ответ

Нет. `city` — свободная строка в профиле пользователя. Frontend может использовать локальные autocomplete-подсказки, но backend не валидирует город по справочнику и пользователь может сохранить собственный вариант.
